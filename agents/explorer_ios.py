"""
Agente 0 — Explorador iOS (XCUITest)

Responsabilidad:
- Conecta Appium al iPhone físico vía XCUITest
- Navega la app sistemáticamente (BFS sobre el grafo de pantallas)
- Guarda apps/{app_id}/ui_map_ios.json con todos los elementos encontrados

Uso:
    APP_ID=tvnPass IOS_BUNDLE_ID=com.tvn-2.appletv python agents/explorer_ios.py
"""
...
def _find_safe(driver, xpath: str, limit: int = 20, timeout: int = 8) -> list:
    """find_elements con timeout — evita colgar en pantallas de video."""
    with ThreadPoolExecutor(max_workers=1) as ex:
        fut = ex.submit(driver.find_elements, AppiumBy.XPATH, xpath)
        try:
            return fut.result(timeout=timeout)[:limit]
...
def screen_signature(driver) -> str:
    try:
        names = set()
        for xpath in ["//XCUIElementTypeButton", "//XCUIElementTypeStaticText"]:
            for e in _find_safe(driver, xpath, limit=15):
...
def capture_elements(driver) -> list[dict]:
    ...
    for xpath, etype in [
        ...
        for el in _find_safe(driver, xpath):
            try:
...
def connect_appium(bundle_id: str, device_key: str) -> webdriver.Remote:
    ...
    if not caps:
        ...
        caps = {
            ...
            "appium:udid":                 os.environ.get("IOS_DEVICE_UDID", ""),
            "appium:noReset":              True,
            "appium:xcodeOrgId":           os.environ.get("IOS_TEAM_ID", ""),
    ...
    appium_url = os.environ.get("IOS_APPIUM_SERVER_URL", os.environ.get("APPIUM_SERVER_URL", "http://localhost:4724"))
...
def run(app_id: str, bundle_id: str, device_key: str = "iphone_physical") -> None:
    ...
    map_path = app_dir / "ui_map_ios.json"
    map_path.write_text(json.dumps(ui_map, indent=2, ensure_ascii=False))
    print(f"\n✅ ui_map_ios.json guardado en {map_path.relative_to(ROOT)}")
    print(f"\n👉 Siguiente paso: dile a Claude Code que analice apps/{app_id}/ui_map_ios.json")
    print(f"{'═' * 60}\n")
...
if __name__ == "__main__":
    ...
    if not app_id or not bundle_id:
        print("ERROR: APP_ID e IOS_BUNDLE_ID son requeridos.")
        sys.exit(1)
