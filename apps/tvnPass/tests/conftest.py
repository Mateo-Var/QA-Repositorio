...
- `reset_app` scope=class   → terminate + activate una vez por clase (~5 resets vs 19)
...
- PiP dismissal → cierra Picture-in-Picture antes de terminate_app para evitar
  que el overlay del sistema quede flotando y tape elementos del siguiente test
"""
...
BUNDLE_ID   = os.environ.get("APP_BUNDLE_ID", "com.tvn-2.appletv")
...
def _restart_appium():
    ...
    # Esperar hasta que levante (máx 15s)
    for _ in range(15):
        time.sleep(1)
        if _appium_is_alive():
...
def _dismiss_pip(driver) -> bool:
    """
    Cierra el overlay de Picture-in-Picture si está activo.
    Debe llamarse ANTES de terminate_app — el PiP es del sistema y sobrevive
    al cierre de la app, tapando elementos del siguiente test.

    Retorna True si había PiP activo y fue cerrado.
    """
    from appium.webdriver.common.appiumby import AppiumBy
    try:
        # XCUITest expone el botón de cierre de PiP con este accessibility ID
...
def _kill_wda_zombies():
    """Mata procesos WDA/xcodebuild que quedaron vivos del run anterior."""
...
def _create_session(caps: dict) -> webdriver.Remote:
    """
    Crea una sesión Appium con retry progresivo:

    Intento 1 — usePreinstalledWDA: True  (rápido, reutiliza WDA instalado)
    Intento 2 — mata zombies + usePreinstalledWDA: False (rebuild WDA limpio)
    Intento 3 — restart Appium + usePreinstalledWDA: False (opción nuclear)
    """
    strategies = [
...
def appium_server():
    """Verifica que Appium esté vivo antes del run. Si no, lo reinicia."""
...
def driver(appium_server, device_caps):
    """Una sola sesión Appium/WDA para toda la suite. Con retry y WDA recovery."""
...
def reset_app(driver):
    """
    Reinicia la app una vez por clase de test.
    5 resets en vez de 19 → ~5 min vs ~11 min en físico.
    Cierra PiP antes de terminate para evitar que el overlay tape el siguiente test.
    """
    _dismiss_pip(driver)
